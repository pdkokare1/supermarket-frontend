...
let allProducts = []; let cart = []; let selectedDeliveryType = 'Instant'; let allCategories = [];
...
async function fetchProducts() { 
    ...
    } catch(e){ skeletonGrid.innerHTML='<p style="grid-column: span 2; text-align:center;">Failed to connect.</p>'; } 
}

function renderProducts(productsToRender) { 
    storefront.innerHTML = ''; 
    if(productsToRender.length===0){ storefront.innerHTML='<p style="grid-column:span 2;text-align:center;color:#94A3B8;margin-top:40px;">No products found.</p>'; return; } 
    ...
    productsToRender.forEach(product => { 
        ...
        storefront.appendChild(card); updateCardActionUI(product._id); 
    }); 
}
...
function handleSearch(event) { 
    ...
    renderProducts(allProducts.filter(p => {
        ...
        const tagsMatch = p.searchTags ? p.searchTags.toLowerCase().includes(query) : false;
...
function quickAdd(productId) { 
    const p=allProducts.find(p=>p._id===productId); if(!p)return; 
    ...
    updateCardActionUI(productId); updateGlobalCartUI(); 
}
...
function updateGlobalCartUI() { 
    const totalItems=cart.reduce((s,i)=>s+i.qty,0); const subtotal=cart.reduce((s,i)=>s+(i.currentPrice*i.qty),0); 
    if(totalItems>0){ document.getElementById('ribbon-items-count').innerText=`${totalItems} ITEM${totalItems>1?'S':''}`; document.getElementById('ribbon-total-price').innerText=`₹${subtotal}`; cartRibbon.classList.remove('hidden'); }else{ cartRibbon.classList.add('hidden'); } 
...
async function placeOrder() { 
    ...
    if(!name||!phone||!address){showToast('Please fill out all delivery details!');return;} 
    const subtotal=cart.reduce((s,i)=>s+(i.currentPrice*i.qty),0); const finalTotal=subtotal+DELIVERY_FEE; const scheduleTime = selectedDeliveryType === 'Routine' ? document.getElementById('schedule-time').value : 'ASAP'; const checkoutBtn=document.querySelector('.checkout-btn'); checkoutBtn.innerText='Processing...'; checkoutBtn.disabled=true; 
    try { 
        const res=await fetch(`${BACKEND_URL}/api/orders`,{ method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({customerName:name, customerPhone:phone, deliveryAddress:address, items:cart, totalAmount:finalTotal, deliveryType:selectedDeliveryType, scheduleTime:scheduleTime}) }); 
        const result=await res.json(); 
        if(result.success){ localStorage.setItem('dailyPick_activeOrderId', result.orderId); cart=[]; document.getElementById('cust-name').value=''; document.getElementById('cust-phone').value=''; document.getElementById('cust-address').value=''; setDeliveryType('Instant'); renderProducts(allProducts); updateGlobalCartUI(); closeCart(); switchView('orders'); showToast('Order Received! 🚀'); }else{ showToast('Failed to place order.'); } 
    }catch(e){ showToast('Network error.'); }finally{ checkoutBtn.innerText='Place Order'; checkoutBtn.disabled=false; } 
}
...
async function checkOrderStatus() { 
    ...
    try { 
        const res = await fetch(`${BACKEND_URL}/api/orders/${savedOrderId}`); const result = await res.json(); 
        if(result.success){ 
            const order = result.data; const timeString = new Date(order.createdAt).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}); const scheduleBadge = order.deliveryType === 'Routine' ? `<div style="margin-top:12px; font-size:12px; color:#64748B; font-weight:700;">📅 Routine: ${order.scheduleTime}</div>` : `<div style="margin-top:12px; font-size:12px; color:#16A34A; font-weight:700;">⚡ Instant Delivery</div>`; 
            ...
            if(order.status !== 'Dispatched' && !trackingEventSource) {
                ...
                trackingEventSource.onmessage = (event) => { const data = JSON.parse(event.data); if (data.type === 'STATUS_UPDATE') { showToast('🚚 Your order has been dispatched!'); trackingEventSource.close(); trackingEventSource = null; checkOrderStatus(); } };
            }
        } else { trackingContent.innerHTML='<p class="empty-state">Order details could not be found.</p>'; } 
    }catch(e){ trackingContent.innerHTML='<p class="empty-state">Network error checking status.</p>'; } 
}
